...
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/data/medical/staged/d2_truthrl_cpro}"
...
LR="${LR:-1e-6}"
K="${K:-1}"
LAMBDA_FORMAT="${LAMBDA_FORMAT:-1.0}"
LAMBDA_CONSISTENCY="${LAMBDA_CONSISTENCY:-0.5}"
PROJECT_NAME="${PROJECT_NAME:-stage-d-d2}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-truthrl-cpro-d2}"
...
TRAIN_CMD=("${PY_CMD[@]}" -m verl.trainer.main_ppo \
  ...
  +custom_reward_function.reward_kwargs.stage_mode=d2 \
  +custom_reward_function.reward_kwargs.k="${K}" \
  +custom_reward_function.reward_kwargs.enable_format=True \
  +custom_reward_function.reward_kwargs.enable_consistency=True \
  +custom_reward_function.reward_kwargs.lambda_format="${LAMBDA_FORMAT}" \
  +custom_reward_function.reward_kwargs.lambda_consistency="${LAMBDA_CONSISTENCY}" \
...
if is_truthy "${DRY_RUN}" || is_truthy "${PREFLIGHT_ONLY}"; then
  ...
  echo "[DRY-RUN] D2 command was not executed."
  exit 0
fi

"${TRAIN_CMD[@]}"

echo "[PASS] D2 run completed."
