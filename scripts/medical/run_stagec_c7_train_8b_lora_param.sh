...
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/data/medical/stagec_c7_8b_lora_param}"
SFT_CKPT_DIR="${SFT_CKPT_DIR:-${RUN_ROOT}/sft_ckpt}"
REPORT_DIR="${REPORT_DIR:-${RUN_ROOT}/reports}"
REPORT_NAME="${REPORT_NAME:-stageC_c7_train_8b_lora_param_summary.json}"
...
NNODES="${NNODES:-1}"
PROJECT_NAME="${PROJECT_NAME:-stagec-c7-lora-param}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen3-8b-lora-param}"
...
echo "[INFO] Stage C7 8B parameterized LoRA SFT training"
...
echo "[INFO] LORA_RANK=${LORA_RANK} LORA_ALPHA=${LORA_ALPHA} MODEL_DTYPE=${MODEL_DTYPE}"
echo "[INFO] SAVE_FREQ=${SAVE_FREQ} MAX_CKPT_TO_KEEP=${MAX_CKPT_TO_KEEP}"
...
report = {
    ...
    "max_ckpt_to_keep": int("${MAX_CKPT_TO_KEEP}"),
}
...
echo "[PASS] Stage C7 parameterized LoRA training completed."
