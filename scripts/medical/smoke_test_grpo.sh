...
echo "[INFO] Running GRPO import smoke test in Apptainer"
...
apptainer exec "${NV_FLAG[@]}" --cleanenv \
  ...
  "${IMAGE_PATH}" \
  python3 -c "from verl.trainer.main_ppo import main; print('GRPO trainer import OK')"

echo "[PASS] GRPO smoke test succeeded"
